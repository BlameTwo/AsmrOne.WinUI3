...
public interface IAsmrClient
{
    ...
    Task<List<TagList>> GetTagAsync(TagType tags, CancellationToken token = default);
    ...
    Task<WorksResponse> SearchAsync(IEnumerable<SearchTagWrapper> query, WorkOrder order, int page, int pageSize, bool isSubtitle, CancellationToken token = default);
}
