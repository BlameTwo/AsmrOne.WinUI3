...
public partial class NotifyIconWinUI : Control
{
    ...
    public void CreateTrayIcon(string ico, Window win)
    {
        ...
        {
            var hWnd = (nint)win.AppWindow.Id.Value;
            var nid = new NOTIFYICONDATA
            {
                ...
                hIcon = _iconHandle,
                szTip = "Asmr Version 1",
            };
            Shell_NotifyIcon(Shell_NotifyIconType.Add, ref nid);
        }
    }

    public void RegisterWin(Window mainWindow)
    {
        monitor = new WindowMessageMonitor(mainWindow);
    ...
    private void ShowContextMenu()
    {
        ...
        IntPtr hWnd = WindowNative.GetWindowHandle(
            ProgramLife.GetService<IAppSetup<App>>().MainWindow
        );

        if (hWnd == IntPtr.Zero)
        {
            Debug.WriteLine("无效的窗口句柄");
            return;
        }

        Debug.WriteLine($"显示菜单: X={PoInt.X}, Y={PoInt.Y}, hWnd={hWnd}");
        ...
        Debug.WriteLine($"TrackPopupMenuEx 返回值: {cmd}");
        if (cmd != 0)
        {
